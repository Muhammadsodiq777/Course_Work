...
namespace HotelListing.Controllers
{
    ...
    public class HotelController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetAllHotelsPAged([FromQuery] RequestParams request)
        {
            var hotels = await _unitOfWork.Hotels.GetAllPaged(request);
        ...
        //[Authorize]
        [HttpPut("{id:int}")]
        public async Task<IActionResult> UpdateHotel(long id, [FromBody] UpdateHotelDTO hotelDTO)
        {
            if (!ModelState.IsValid)
            {
        ...
        public async Task<IActionResult> DeleteHotel(long id)
        {
            if (id < 1)
            {
                _logger.LogError($"Invalid ID attemp in {nameof(DeleteHotel)}");
                return BadRequest(ModelState);
            }
            var hotel = await _unitOfWork.Hotels.Get(q => q.Id == id, new List<string> { "Countries" });
            if (hotel == null)
